...
| Windows release fork | `v3.0.29`, unsigned |
...
### Интерфейс «Атлас» — 3.0.29

Новый Windows-интерфейс в тёмно-зелёной палитре: планета, явная кнопка
подключения, активный источник, два режима и сворачиваемые быстрые маршруты.
На широком окне подключение и маршруты располагаются рядом; на узком — друг
под другом. Состояния отключения, подключения и ошибки различаются текстом,
а не только цветом. Полный VPN не стирает сохранённые маршруты сервисов.
Раздел **Сервисы** содержит поиск по названию/домену, выбор маршрута и закрепление
на главной. Раздел **Источники VPN** управляет подписками и публичным резервом.
Расширенные инструменты доступны в настройках, логи — через диагностику.
Android сохраняет прежний мобильный интерфейс. Подробности:
...
В версии `3.0.29` для Discord рекомендуется `Через VPN`. Встроенный Zapret
...
[`v3.0.29`](https://github.com/sunnydjam/dropo-by-sunnydjam/releases/tag/v3.0.29).
...
отключить в разделе `Настройки → Обновления`. Portable-версия показывает новый
